...
def group_by_first_letter(words):
    ...
    for word in words:
        first_letter = word[0]
        group.setdefault(first_letter, []).append(word)
    return group
    pass
...
def calculate_grades_average(students):
    ...
    for name, grades in students.items():
        if grades:
            ...
            grades_average[name] = round(avg, 2)
        else:
            grades_average[name] = 0.0  # or None, depending on preference
...
def nested_dict_access(data, keys):
    ...
    new_data = data
    for k in keys:
        if isinstance(new_data, dict) and k in new_data:
            new_data = new_data[k]
        else:
            return None
    return new_data
